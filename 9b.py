...
params = {'world_size':(10,10),
          ...
          'same_pref' :0.4,
          'max_iter'  :100,
          'out_path'  :r'/Users/maryell/Desktop/9b.csv' }
...
class Dragons():
    ...
    def move(self):
        speed = self.am_i_fast_enough()
        
        if not speed:
            empty = self.world.find_vacant(return_all=True)
            for one in empty:
                i_moved = False
                is_it_good = self.am_i_fast_enough()
                if is_it_good is True:
                    self.world.grid[self.location] = None
                    self.location = one
                    self.world.grid[one] = self
                    i_moved = True
                    return 'fast'
                if i_moved is False:
                    return 'slow'
    ...
    def am_i_fast_enough(self, loc=False, neighbor_check=False):
        if not loc:
